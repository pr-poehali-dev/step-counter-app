...
type Tab = "home" | "history" | "goals" | "achievements" | "social" | "settings";
...
const friends = [
  { id: 1, name: "Алексей К.", avatar: "АК", steps: 9850, rank: 1, color: "#f97316" },
  { id: 2, name: "Мария С.", avatar: "МС", steps: 9200, rank: 2, color: "#a855f7" },
  { id: 3, name: "Вы", avatar: "ВЫ", steps: 8742, rank: 3, color: "#00f5a0", isMe: true },
  { id: 4, name: "Дмитрий П.", avatar: "ДП", steps: 7100, rank: 4, color: "#3b82f6" },
  { id: 5, name: "Анна В.", avatar: "АВ", steps: 5430, rank: 5, color: "#ec4899" },
];
...
function CircularProgress({ value, max, size = 240 }: { value: number; max: number; size?: number }) {
...
function BarChart({ data, maxSteps, showDay }: {
  data: BarItem[];
  maxSteps: number;
  showDay?: boolean;
}) {
...
function HistoryScreen() {
  ...
  const data = period === "week" ? weekData : monthData.map(d => ({ ...d, day: String(d.day) }));
          ...
          {[...data].reverse().map((d, i) => (
            <div key={i} className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-sm font-bold font-golos ${d.steps >= d.goal ? "text-neon-green" : "text-white"}`} style={{ color: d.steps >= d.goal ? "#00f5a0" : undefined }}>
